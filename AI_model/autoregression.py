...
def boxcox_ar(dates, observations, split):
    ...
    for idx in range(len(predicted)):
        if idx == 0:
            boxcox_forecast = predicted[idx] + train_group.iloc[-1]
            boxcox_test = test_group[idx] + train_group.iloc[-1]
        else:
...
def autoreg(dates, group, split):
    ...
    selector = ar_select_order(train_group, 24)
    model = AutoReg(train_group, lags=selector.ar_lags).fit()
